...
#REFERENCES:
#
# [1] https://cartopy.readthedocs.io/stable/gallery/miscellanea/tube_stations.html#sphx-glr-gallery-miscellanea-tube-stations-py

## Loading data
def load_air_quality_data():
    file_path = pl.Path('COMPILED_DATA') / 'air_quality_monitoring_results_2003_to_2024_1.xlsx'
...
def load_predicted_data():
    file_path_2021 = pl.Path('COMPILED_DATA') / 'sheffield_2021_prediction.csv'
    file_path_2024 = pl.Path('COMPILED_DATA') / 'sheffield_2024_prediction.csv'
    file_path_2040 = pl.Path('COMPILED_DATA') / 'sheffield_2040_prediction.csv'
...
def plot_site_locations(dataframe):
    """ Plots the locations of the sites on a map using cartopy. Based on this [1] Cartopy example """
    imagery = OSM()
...
def plot_site_locations_colour_by_ward(dataframe):
    """ Plots the locations of the sites on a map using cartopy. 
    The colour of each point indicates the council ward. """
...
def plot_sites_where_over_2005_limit(dataframe):
    """
    Plots only the sites where any year exceeds the 2005 WHO annual mean guideline of 40 µg/m³.
    """
...
def plot_1km_grid_coloured_by_no2(dataframe, no2_col, year):
    ...
    for xi, yi, n in zip(xs, ys, no2):
        ...
        if n < 10: # within 2021 WHO guideline
            color = 'green'
        elif n < 40: # within 2005 WHO guideline
            color = 'yellow'
        else: # above 2005 guideline
            color = 'red'
        ...
        # add text for NO2 level, rounded to 1 decimal place
        ax.text(
            xi, yi, f"{n:.1f}",
...
## Difference in NO2 levels by council ward
